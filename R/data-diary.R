...
upcoming_stats <- gov_uk |>
  ...
  bind_rows(
    eurostat |> mutate(source = "Eurostat", country = "European Union")
  ) |>
  ...
  mutate(
    ...
    globalwitness = grepl(globalwitness_keywords, title, ignore.case = T) |
      source %in% globalwitness_sources
  ) |>
...
write_sheets_cal <- function() {
  calendar_google_sheets <- upcoming_stats |>
    filter(
      date >= lubridate::floor_date(Sys.Date()),
      # date < lubridate::ceiling_date(Sys.Date() %m+% months(1), "month"),
      globalwitness
    ) |>
    mutate(
  ...
  range_clear(
    "https://docs.google.com/spreadsheets/d/1SAPy0tfzRN66ngdblNeEFhbGy8Q5V96C0DeC9qRo6Wc/edit#gid=891834841",
    sheet = "Full Schedule",
    range = "B4:F"
  )
...
write_ical <- function() {
  ...
  export_calendar(
    upcoming_stats,
    file = "output/globalwitness_cal.ics",
    tz = "GMT"
  )
}
